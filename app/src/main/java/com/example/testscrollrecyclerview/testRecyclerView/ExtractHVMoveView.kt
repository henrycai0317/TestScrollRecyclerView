...
class ExtractHVMoveView (val mLayoutView:View){
...
//    fun setHorizontal
    ...
    private fun verticalShadowControl(pRecyclerView: RecyclerView, pHrShadowView: View) {
        pRecyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                super.onScrolled(recyclerView, dx, dy)
                if (pRecyclerView.canScrollVertically(-1)) {
                    pHrShadowView.visibility = RelativeLayout.VISIBLE
                } else {
                    pHrShadowView.visibility = RelativeLayout.INVISIBLE
                }
            }
        })
    }
//
//    fun setOnTouchEvent(pEvent:MotionEvent?):Boolean{
//        when (pEvent?.action) {
//            MotionEvent.ACTION_DOWN -> return true
//            MotionEvent.ACTION_MOVE -> {
//                val offsetX = abs(pEvent.x - mStartX).toInt()
//                if (offsetX >= 0) {
//                    mMoveOffsetX = (mStartX - pEvent.x + mMaxFixX).toInt()
//                    if (0 > mMoveOffsetX) {
//                        mMoveOffsetX = 0
//                    } else {
//
...
//        }
//
//        return mLayoutView.onTouchEvent(pEvent)
//    }
